...
int main(int argc, char **argv) {
  ...
  unsigned long duration, printStep;
	...
	idGroup = myopt_AddGroup
	(parser, "Gruppo 0", false, false);
	...
	strcpy
	(strDescr, "Mersenne Twister initialization.");
	...
	if (idOption >= 0 &&
	    parser->arrayOptArgs[idOption].countOccurrences
	    > 0 ) {
	...
	if (parser->arrayOptArgs[idOption].countArgs > 0 ) {
		seed = (unsigned)
		parser->arrayOptArgs[idOption].arrayArgs[0].intValue;
	}

	idOption = myopt_LookupLong(parser, "distribParticles");	
	gridPart = (partsCounter)
	parser->arrayOptArgs[idOption].arrayArgs[0].intValue;
	...
	printStep = (unsigned long)
	parser->arrayOptArgs[idOption].arrayArgs[0].intValue;

	idOption = myopt_LookupLong(parser, "glue");	
	glue = (double)
	parser->arrayOptArgs[idOption].arrayArgs[0].floatValue;
